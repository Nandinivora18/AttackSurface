"""
OWASP A02:2025 — Security Misconfiguration Assessment Module
============================================================
...
async def assess_a02_misconfiguration(
    target_url: str,
    ...
    """
    Aggregates and executes A02:2025 Security Misconfiguration assessment.
    """
    ...
    if probe_active and target_url:
        ...
        if is_safe:
            try:
                ...
                logger.debug(f"A02 OPTIONS check error: {e}")
        ...
        async with httpx.AsyncClient(timeout=4.0, follow_redirects=False) as client:
            for d in COMMON_STATIC_DIRS:
                ...
                try:
                    ...
                    logger.debug(f"A02 directory listing probe error for {dir_url}: {e}")

    limitations = (
        "Evaluates external HTTP security headers, CORS policies, DNS records (SPF/DMARC), exposed directory indexing, "
        "and advertised HTTP methods to provide direct externally observable evidence of the evaluated security configuration. "
        "Does not audit internal OS, container configs, or private network boundaries."
    )
